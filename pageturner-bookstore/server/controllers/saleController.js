...
exports.getAllSales = async (req, res) => {
  try {
    const [sales] = await db.query(`
      SELECT s.*, b.title as book_title, c.name as customer_name 
      FROM sales s
...
exports.createSale = async (req, res) => {
  try {
    ...
    const { book_id, quantity, customer_id } = req.body;
    ...
    if (!book_id || !quantity || quantity <= 0) {
      return res.status(400).json({ 
        message: 'Book ID and quantity (> 0) are required' 
      });
    }
    ...
    const bookPrice = typeof book[0].price === 'string' 
      ? parseFloat(book[0].price) 
      : book[0].price;
    ...
    const [result] = await db.query(
      'INSERT INTO sales (customer_id, book_id, quantity, total_price) VALUES (?, ?, ?, ?)',
      [effectiveCustomerId, book_id, quantity, total_price]
    );
